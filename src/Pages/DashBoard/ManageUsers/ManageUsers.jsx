    ...
    <div className="overflow-x-auto">
      <table className="min-w-full border border-gray-200 bg-white shadow-lg">
        ...
        <tbody>
          {/* Table rows */}
          {users.map((user) => 
          <tr key={user._id} className="h-[70px] border-b bg-[#484D58] text-[#FFFFFF]">
          <th className="px-6 py-4 text-start ">{user.name}</th>
          <th className="px-6 py-4 text-start ">{user.email}</th>
          {user.role === 'admin' ?
          ...
          :
          <th className="px-6 py-4 text-start">
            <button onClick={()=> handleAdmin(user)} className="flex items-center rounded-full bg-[#66451c] px-4 py-2 font-bold text-white shadow-md transition-all duration-300 hover:bg-blue-700 gap-2 hover:cursor-pointer">
            <RiAdminFill />
              Make Admin
            </button>
          </th>
